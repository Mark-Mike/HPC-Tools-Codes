...
int my_dgesv(int n, int nrhs, double *a, int lda, int *ipiv, double *b, int ldb)
{
  ...
  double matrix[n][n+1];
  printf("HI\n");
  #pragma omp parallel default(none) private(i,j,k) firstprivate(ratio) shared(u,matrix,n,lda,ldb,a,b,x_matrix,nrhs) reduction(+:x) //num_threads(4)
  #pragma omp for schedule(dynamic, 1) nowait //reduction(+:x)
  for (u=0;u<ldb;u++)
  {
	for (i=0;i<n;i++)
	{
	...
	//#pragma omp parallel default(none) private(i,j,k) firstprivate(ratio) shared(n,lda,matrix)
	...
	for(i=0;i<n-1;i++)
	{
	  ...
	  //#pragma omp parallel default(none) private(j,k) firstprivate(ratio) shared(i,n,lda,matrix)
